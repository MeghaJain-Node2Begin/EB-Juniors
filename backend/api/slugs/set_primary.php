...
if (!empty($data->slug_id)) {
    try {
        ...
        // Set all to false
        $query1 = "UPDATE course_slug SET is_primary = FALSE";
        $pdo->exec($query1);

        // Set the chosen one to true
        $query2 = "UPDATE course_slug SET is_primary = TRUE WHERE slug_id = :slug_id";
        ...
        if ($templateRow) {
            ...
            // Prepare update statement for courses
            $updateCourseStmt = $pdo->prepare("UPDATE courses SET slug_title = :slug_title WHERE course_id = :course_id");
            ...
            foreach ($courses as $course) {
                ...
                $updateCourseStmt->bindValue(':course_id', $courseId);
