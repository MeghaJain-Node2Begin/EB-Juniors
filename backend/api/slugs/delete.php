...
if (!empty($data->slug_id)) {
    try {
        ...
        $templateStmt = $pdo->prepare("SELECT template_rule FROM course_slug WHERE slug_id = :slug_id");
        ...
        if ($templateRow) {
            ...
            // Fetch all courses
            $courseStmt = $pdo->query("SELECT course_id, course_name, slug_title FROM courses");
            $courses = $courseStmt->fetchAll(PDO::FETCH_ASSOC);
            
            $updateCourseStmt = $pdo->prepare("UPDATE courses SET slug_title = '' WHERE course_id = :course_id");
            
            foreach ($courses as $course) {
                if (!empty($course['slug_title'])) {
                    $slugifiedName = strtolower(trim(preg_replace('/[^A-Za-z0-9-]+/', '-', $course['course_name']), '-'));
                    ...
                    if ($course['slug_title'] === $expectedSlug) {
                        $updateCourseStmt->bindValue(':course_id', $course['course_id']);
        ...
        $query = "DELETE FROM course_slug WHERE slug_id = :slug_id";
