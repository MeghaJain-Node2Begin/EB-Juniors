...
if (!empty($data->slug_id) && !empty($data->template_rule)) {
    try {
        $query = "UPDATE course_slug SET template_rule = :template_rule WHERE slug_id = :slug_id";
        ...
        if ($stmt->execute()) {
            ...
            $checkPrimaryStmt = $pdo->prepare("SELECT is_primary FROM course_slug WHERE slug_id = :slug_id");
            ...
            if ($checkPrimaryStmt->fetchColumn()) {
                ...
                foreach ($courses as $c) {
                    ...
                    $updateCourseStmt->bindValue(':course_id', $c['course_id']);
