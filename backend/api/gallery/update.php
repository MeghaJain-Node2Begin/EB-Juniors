...
if (!empty($data->image_id) && !empty($data->title)) {
    try {
        ...
        if ($stmt->execute()) {
            ...
            echo json_encode(["success" => false, "message" => "Unable to update gallery item."]);
        }
    ...
    echo json_encode(["success" => false, "message" => "Unable to update gallery item. Data is incomplete."]);
}
