...
export default function SlugTemplatePage() {
  ...
  const fetchTemplates = async () => {
    setIsLoading(true);
    try {
      const response = await fetch('http://localhost:8000/api/slugs/read.php');
  ...
  const handleAddTemplate = async () => {
    ...
    try {
      ...
      const url = isEditing 
        ? 'http://localhost:8000/api/slugs/update.php' 
        : 'http://localhost:8000/api/slugs/create.php';
  ...
  const handleDelete = async (id: number) => {
    ...
    try {
      const response = await fetch('http://localhost:8000/api/slugs/delete.php', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  const handleSetPrimary = async (id: number) => {
    setIsProcessing(true);
    try {
      const response = await fetch('http://localhost:8000/api/slugs/set_primary.php', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ slug_id: id }),
      });
  ...
  return (
    <div className="relative">
      <div className="flex items-center justify-between mb-8">
        <div>
          <h1 className="text-3xl font-black text-zinc-900 tracking-tight">Slug Templates</h1>
          <p className="text-zinc-500 font-medium mt-1">Manage global rules for course URL generation</p>
        </div>
        <button 
          onClick={() => {
            setMessage({ text: '', type: '' });
