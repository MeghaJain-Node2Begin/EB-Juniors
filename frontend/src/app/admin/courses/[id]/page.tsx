...
import { ArrowLeft, BookOpen, Clock, Target, Star, AlignLeft, ShieldCheck, IndianRupee, Link as LinkIcon, GraduationCap } from 'lucide-react';
...
export default function CourseDetailsPage() {
    ...
    return (
      <div className="min-h-[calc(100vh-2rem)] flex items-center justify-center relative overflow-hidden bg-zinc-50 rounded-[32px] m-4 mt-0">
        <div className="text-center">
          ...
          <p className="text-zinc-500 font-medium mb-8 max-w-md">{error || 'Course not found.'}</p>
          <button 
            onClick={() => router.back()}
  ...
  return (
    <div className="min-h-[calc(100vh-2rem)] relative overflow-hidden bg-[#FAFAFA] rounded-[32px] m-4 mt-0">
      ...
      <div className="absolute bottom-[-10%] left-[-10%] w-[500px] h-[500px] bg-indigo-100/40 rounded-full blur-[100px] pointer-events-none" />
            ...
            <div>
              <div className="flex items-center gap-4 mb-3">
                ...
                {courseData.is_featured && (
                  <span className="px-4 py-1.5 rounded-full text-xs font-bold tracking-widest uppercase shadow-sm border bg-amber-500 text-white border-amber-400 flex items-center gap-1">
              ...
              <p className="text-zinc-500 font-medium flex items-center gap-2 text-base tracking-wide">
                ...
                Target Class: <strong className="text-zinc-800 font-bold">{courseData.class_name || 'General / All Classes'}</strong>
              </p>
            </div>
          </div>
        </motion.div>
        ...
        >
          ...
          <div className="lg:col-span-8 flex flex-col gap-6">
            ...
            <motion.div variants={itemVariants} className="bg-white/60 backdrop-blur-xl rounded-[32px] p-8 md:p-12 shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-white transition-all duration-300 hover:shadow-[0_8px_30px_rgb(0,0,0,0.08)]">
              ...
              <p className="text-zinc-600 leading-[1.8] text-lg font-medium whitespace-pre-wrap">
                {courseData.short_description || 'No short overview available.'}
              </p>
            </motion.div>

            {/* Full Description Glass Card */}
          ...
          <div className="lg:col-span-4 flex flex-col gap-6">
            ...
            <div className="grid grid-cols-2 gap-6">
              <motion.div variants={itemVariants} className="bg-white/60 backdrop-blur-xl rounded-[32px] p-6 shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-white flex flex-col gap-4 group hover:bg-white/80 transition-colors cursor-default">
                ...
                <div>
                  ...
                  <h3 className="text-zinc-900 font-black text-lg tracking-tight leading-none">{courseData.level || 'N/A'}</h3>
                </div>
              </motion.div>
              ...
              <motion.div variants={itemVariants} className="bg-white/60 backdrop-blur-xl rounded-[32px] p-6 shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-white flex flex-col gap-4 group hover:bg-white/80 transition-colors cursor-default">
                ...
                <div>
                  ...
                  <h3 className="text-zinc-900 font-black text-lg tracking-tight leading-none">{courseData.duration || 'N/A'}</h3>
                </div>
              </motion.div>
            </div>
            ...
            <motion.div variants={itemVariants} className="bg-white/60 backdrop-blur-xl rounded-[32px] p-8 shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-white flex items-center gap-6 group hover:bg-white/80 transition-colors cursor-default">
              ...
              <div>
                <p className="text-xs font-bold text-zinc-400 uppercase tracking-widest mb-1.5">Course Fees</p>
                <h3 className="text-zinc-900 font-black text-2xl tracking-tight">
                  {courseData.fees ? `₹${courseData.fees}` : 'Free'}
                </h3>
              </div>
            </motion.div>
            ...
            <motion.div variants={itemVariants} className="bg-white/60 backdrop-blur-xl rounded-[32px] p-8 shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-white flex items-center gap-6 group hover:bg-white/80 transition-colors cursor-default">
              ...
              <div className="overflow-hidden">
                <p className="text-xs font-bold text-zinc-400 uppercase tracking-widest mb-1.5">URL Slug</p>
                <h3 className="text-zinc-900 font-bold text-base tracking-tight truncate">{courseData.slug || 'N/A'}</h3>
              </div>
            </motion.div>
