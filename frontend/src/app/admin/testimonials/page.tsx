...
import { Plus, X, Save, Edit2, Trash2, MessageSquareQuote, Star, MoreVertical } from 'lucide-react';
...
export default function ManageTestimonials() {
  ...
  useEffect(() => {
    if (isModalOpen || testimonialToDelete) {
  ...
  const openEditModal = (t: TestimonialData) => {
    setEditingTestimonialId(t.testimonial_id);
    setStudentName(t.student_name);
    setParentName(t.parent_name || '');
    setReview(t.review);
    setRating(t.rating?.toString() || '5');
    setApproved(Boolean(t.approved));
    setMetaTitle(t.meta_title || '');
    setMetaDescription(t.meta_description || '');
    setMetaKeywords(t.meta_keywords || '');
    setOgTitle(t.og_title || '');
    setOgDescription(t.og_description || '');
    setSchemaMarkup(t.schema_markup || '');
    setActiveTab('details');
    setMessage({ text: '', type: '' });
    setIsModalOpen(true);
  };

  const confirmDelete = (t: TestimonialData) => {
  ...
  const executeDelete = async () => {
    ...
    setIsDeleting(true);
    try {
      const response = await fetch('http://localhost:8000/api/testimonials/delete.php', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ testimonial_id: testimonialToDelete.testimonial_id }),
      });
      const data = await response.json();
  ...
  const handleGenerateSEO = async () => {
    if (!studentName || !review) {
      alert("Please enter a Student Name and Review first to generate SEO data.");
      return;
    }
    ...
    try {
      const response = await fetch('http://localhost:8000/api/ai/generate_seo.php', {
        ...
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          entity_type: 'Student Testimonial',
          context: `Student: ${studentName}, Parent: ${parentName}, Rating: ${rating}/5, Review: "${review}"`
        })
      });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!studentName || !review) {
      setMessage({ text: 'Student Name and Review are required.', type: 'error' });
    ...
    try {
      const url = editingTestimonialId 
        ? 'http://localhost:8000/api/testimonials/update.php' 
        : 'http://localhost:8000/api/testimonials/create.php';
      ...
      if (editingTestimonialId) {
        payload.testimonial_id = editingTestimonialId;
      }
      ...
      if (data.success) {
        setMessage({ 
          text: editingTestimonialId ? 'Testimonial updated successfully!' : 'Testimonial created successfully!', 
          type: 'success' 
        });
        ...
        setMessage({ text: data.message || 'Failed to save testimonial.', type: 'error' });
      }
    } catch (error) {
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-8">
        <div>
          <h1 className="text-3xl font-black text-zinc-900 tracking-tight">Manage Testimonials</h1>
          <p className="text-zinc-500 font-medium mt-1">Review and showcase student feedback</p>
        </div>
        <button 
      ...
      {isLoading ? (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {[1, 2, 3].map((skeleton) => (
            <div key={skeleton} className="bg-white rounded-3xl p-6 border border-zinc-100 shadow-sm animate-pulse h-48">
              ...
              <div className="w-full h-16 bg-zinc-100 rounded-lg mb-4"></div>
              <div className="w-24 h-4 bg-zinc-50 rounded-lg"></div>
            </div>
          ))}
        </div>
        ...
        <div className="bg-white rounded-3xl shadow-sm border border-zinc-100 p-12 flex flex-col items-center justify-center text-center">
          ...
          <h3 className="text-xl font-bold text-zinc-900 mb-2">No Testimonials Found</h3>
          <p className="text-zinc-500 font-medium max-w-sm mb-6">You don't have any student testimonials yet. Add one to build trust.</p>
            ...
            >
              ...
              <div className="flex justify-between items-start mb-4">
                <div className="flex items-center gap-3">
                  <div className="w-12 h-12 bg-teal-50 text-teal-600 rounded-full flex items-center justify-center font-black text-xl shadow-sm group-hover:scale-110 transition-transform duration-300">
                    {testimonial.student_name.substring(0, 1)}
                  </div>
                  <div>
                    <h3 className="text-lg font-black text-zinc-900">{testimonial.student_name}</h3>
                    <p className="text-teal-600 text-xs font-bold uppercase tracking-wider">{testimonial.parent_name || 'Student'}</p>
                  </div>
                </div>
              ...
              <div className="flex items-center gap-1 mb-4">
                {[...Array(5)].map((_, i) => (
                  <Star 
                    key={i} 
                    className={`w-4 h-4 ${i < testimonial.rating ? 'text-yellow-400 fill-yellow-400' : 'text-zinc-200'}`} 
                  />
                ))}
              </div>
              
              <p className="text-zinc-600 text-sm font-medium leading-relaxed italic line-clamp-4 mb-6 flex-1">
                "{testimonial.review}"
              </p>
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-zinc-900/40 backdrop-blur-sm">
          ...
          >
            ...
            <div className="p-6 overflow-y-auto flex-1 min-h-0 overscroll-y-contain" data-lenis-prevent>
              ...
              <form id="add-testimonial-form" onSubmit={handleSubmit} className="space-y-6">
                <div className={activeTab === 'details' ? "grid grid-cols-1 md:grid-cols-2 gap-6" : "hidden"}>
                  <div className="col-span-2 md:col-span-1">
                    <label className="block text-sm font-bold text-zinc-700 mb-2">Student Name</label>
                    <input 
                      type="text" 
                      value={studentName}
                  ...
                  <div className="col-span-2 md:col-span-1">
                    <label className="block text-sm font-bold text-zinc-700 mb-2">Parent Name (Optional)</label>
                    <input 
                      type="text" 
                      value={parentName}
                  ...
                  <div className="col-span-2 md:col-span-1">
                    <label className="block text-sm font-bold text-zinc-700 mb-2">Rating (1-5)</label>
                    <input 
                      type="number" 
                      min="1" max="5"
                      value={rating}
                  ...
                  <div className="col-span-2 md:col-span-1 flex flex-col justify-center">
                    <label className="flex items-center gap-3 cursor-pointer mt-6">
                      ...
                      />
                      <span className="text-sm font-bold text-zinc-700">Approved?</span>
                    </label>
                  </div>
                  
                  <div className="col-span-2">
                    <label className="block text-sm font-bold text-zinc-700 mb-2">Review</label>
                    <textarea 
                      value={review}
            ...
            <div className="p-6 border-t border-zinc-100 flex justify-end gap-3 bg-zinc-50/50">
              ...
              >
                ...
                {isSubmitting ? 'Saving...' : (editingTestimonialId ? 'Update Testimonial' : 'Save Testimonial')}
              </button>
            </div>
          </div>
        </div>
        ...
        <div className="fixed inset-0 z-[110] flex items-center justify-center p-4 bg-zinc-900/40 backdrop-blur-sm">
          ...
          >
            <div className="p-6 text-center">
              ...
              <h2 className="text-2xl font-black text-zinc-900 mb-2">Delete Testimonial?</h2>
              <p className="text-zinc-500 font-medium">
                Are you sure you want to delete the testimonial from <span className="font-bold text-zinc-900">"{testimonialToDelete.student_name}"</span>? This action cannot be undone.
              </p>
            </div>
