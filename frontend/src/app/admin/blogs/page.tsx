...
import { Plus, X, Save, Edit2, Trash2, FileText, Clock, MoreVertical, UploadCloud } from 'lucide-react';
...
export default function ManageBlogs() {
  ...
  const fetchCategories = async () => {
    try {
      const response = await fetch('http://localhost:8000/api/blog_categories/read.php');
  ...
  const fetchBlogs = async () => {
    try {
      setIsLoading(true);
      const response = await fetch('http://localhost:8000/api/blogs/read.php');
      const data = await response.json();
      if (data.success) {
        setBlogs(data.data);
      }
  ...
  const openAddModal = () => {
    ...
    setCategoryId(categoriesList.length > 0 ? categoriesList[0].category_id.toString() : '');
    setTitle('');
    setSlug('');
    setAuthorName('');
    setStatus('draft');
  ...
  const openEditModal = (b: BlogData) => {
    ...
    setThumbnailImage(b.thumbnail_image || '');
    setMetaKeywords(b.meta_keywords || '');
    setOgTitle(b.og_title || '');
    setOgDescription(b.og_description || '');
    setSchemaMarkup(b.schema_markup || '');
    setActiveTab('details');
    setMessage({ text: '', type: '' });
    setIsModalOpen(true);
  };

  const confirmDelete = (b: BlogData) => {
    setBlogToDelete(b);
  };
  ...
  const executeDelete = async () => {
    ...
    setIsDeleting(true);
    try {
      const response = await fetch('http://localhost:8000/api/blogs/delete.php', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ blog_id: blogToDelete.blog_id }),
      });
      const data = await response.json();
  ...
  const handleGenerateSEO = async () => {
    if (!title) {
      alert("Please enter a Blog Title first to generate SEO data.");
      return;
    }
    ...
    try {
      const response = await fetch('http://localhost:8000/api/ai/generate_seo.php', {
        ...
        body: JSON.stringify({
          ...
          context: `Title: ${title}, Category ID: ${categoryId}, Short Description: ${shortDescription}, Content: ${content}`
        })
      });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const url = editingBlogId 
        ? 'http://localhost:8000/api/blogs/update.php' 
        : 'http://localhost:8000/api/blogs/create.php';
      ...
      if (data.success) {
        setMessage({ 
          text: editingBlogId ? 'Blog updated successfully!' : 'Blog created successfully!', 
          type: 'success' 
        });
        ...
        setMessage({ text: data.message || 'Failed to save blog.', type: 'error' });
      }
    } catch (error) {
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-8">
        <div>
          <h1 className="text-3xl font-black text-zinc-900 tracking-tight">Manage Blogs</h1>
          <p className="text-zinc-500 font-medium mt-1">Publish and manage articles</p>
        </div>
        <button 
      ...
      {isLoading ? (
        ...
        <div className="bg-white rounded-3xl shadow-sm border border-zinc-100 p-12 flex flex-col items-center justify-center text-center">
          ...
          <h3 className="text-xl font-bold text-zinc-900 mb-2">No Blogs Found</h3>
          <p className="text-zinc-500 font-medium max-w-sm mb-6">You haven't written any blogs yet. Create one to get started.</p>
            ...
            >
              ...
              {blog.thumbnail_image && (
                <div className="w-full h-32 rounded-2xl mb-4 overflow-hidden bg-zinc-100 border border-zinc-100 relative z-10">
                  ...
                  <img src={blog.thumbnail_image.startsWith('http') ? blog.thumbnail_image : `/uploads/blogs/${blog.thumbnail_image}`} alt={blog.title} className="w-full h-full object-cover" />
                </div>
              )}

              <h3 className="text-xl font-black text-zinc-900 mb-1 line-clamp-2 relative z-10" title={blog.title}>{blog.title}</h3>
              <p className="text-teal-600 text-xs font-bold uppercase tracking-wider mb-3">{blog.category_name || 'Uncategorized'}</p>
              <p className="text-zinc-500 text-sm font-medium leading-relaxed line-clamp-2 mb-6 flex-1">
                {blog.short_description || 'No description provided.'}
              </p>
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-zinc-900/40 backdrop-blur-sm">
          ...
          >
            ...
            <div className="p-6 overflow-y-auto flex-1 min-h-0 overscroll-y-contain" data-lenis-prevent>
              ...
              <form id="add-blog-form" onSubmit={handleSubmit} className="space-y-6">
                <div className={activeTab === 'details' ? "grid grid-cols-1 md:grid-cols-2 gap-6" : "hidden"}>
                  ...
                  <div className="col-span-2 md:col-span-1">
                    ...
                    <input 
                      ...
                      placeholder="e.g. top-10-programming-languages"
                      className="w-full p-3.5 bg-zinc-50 border border-zinc-200 rounded-xl focus:bg-white focus:ring-2 focus:ring-teal-500/20 focus:border-teal-500 outline-none transition-all placeholder:text-zinc-400 font-medium text-zinc-800"
                    />
                  </div>

                  <div className="col-span-2 md:col-span-1">
                    <label className="block text-sm font-bold text-zinc-700 mb-2">Author Name</label>
                    <input 
                      type="text" 
                      value={authorName}
                  ...
                  <div className="col-span-2 md:col-span-1 flex flex-col justify-center">
                    <label className="flex items-center gap-3 cursor-pointer mt-6">
                      ...
                      />
                      <span className="text-sm font-bold text-zinc-700">Is Featured?</span>
                    </label>
                  </div>
                  ...
                  <div className="col-span-2">
                    <div className="flex items-center justify-between mb-2">
                      <label className="block text-sm font-bold text-zinc-700">Content</label>
                      <button 
                        type="button" 
                        onClick={() => handleConvertToHTML(content, setContent)}
                    ...
                    <textarea 
                      ...
                      rows={6}
                      placeholder="Enter full blog content (HTML or Markdown)..."
                      className="w-full p-3.5 bg-zinc-50 border border-zinc-200 rounded-xl focus:bg-white focus:ring-2 focus:ring-teal-500/20 focus:border-teal-500 outline-none transition-all placeholder:text-zinc-400 font-medium text-zinc-800 resize-y"
                    />
                  </div>
                </div>
            ...
            <div className="p-6 border-t border-zinc-100 flex justify-end gap-3 bg-zinc-50/50">
              ...
              >
                ...
                {isSubmitting ? 'Saving...' : (editingBlogId ? 'Update Blog' : 'Save Blog')}
              </button>
            </div>
          </div>
        </div>
        ...
        <div className="fixed inset-0 z-[110] flex items-center justify-center p-4 bg-zinc-900/40 backdrop-blur-sm">
          ...
          >
            <div className="p-6 text-center">
              ...
              <h2 className="text-2xl font-black text-zinc-900 mb-2">Delete Blog?</h2>
              <p className="text-zinc-500 font-medium">
                Are you sure you want to delete <span className="font-bold text-zinc-900">"{blogToDelete.title}"</span>? This action cannot be undone.
              </p>
            </div>
